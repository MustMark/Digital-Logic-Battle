...
architecture Behavioral of delay_3sec is
    signal counter    : integer range 0 to 75000000 := 0;  -- Counter for 3 seconds
...
begin
    ...
    begin
        if rising_edge(clk) then
            -- Check if either player has a valid action (not "00")
            if (p1_action /= "00") and (p2_action /= "00") then
                ...
                delay_flag <= '0';  -- Reset the flag if no valid action is given
            end if;
        end if;
    end process;
